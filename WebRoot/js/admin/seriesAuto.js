...
function autoComplate2(el){
	$.post(AUTOURL,"key="+el.value.trim(),function(data) {
		$(".autoComplate a").remove();
		$(".autoComplate").show();
		...
		$(".searcRresult").html(data.length);
			...
			for(var temp in data){
				fillAutoComplateData2(temp,data);
			}
			$(".autoComplate a").unbind();
			$(".autoComplate a").mouseover(function(){
				$(".autoComplate a").removeClass("active");
...
function fillAutoComplateData2(index,data){
	if(index=='length')return;
	$(".autoComplate p").before($("<a class='list-group-item' name='"+data[index]+"' id='"+index+"'>"+(data[index])+"</a>"));
}