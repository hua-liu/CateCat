                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        if (selected.length <= 0) {
                        	$.post("json/category_add","category.name="+$(createInput).find('input').val(),function(data){
                        		if(data!=null){
                        	...
                        	//添加分类
                        	var data = "category.name="+$(createInput).find('input').val()+"&category.parent.id="+$(selected).attr("data-id");
                        	$.post("json/category_add",data,function(data){
                        		if(data!=null){
                        			if(!(data instanceof Object)) data = eval("("+data+")");
	                        		if(data.result){
	                        			$(item).attr("data-id",data.id);
                       ...
                       /* $(easyTree).prepend(warningAlert);
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                	//向数据库更新分类名字
                                	$.post("json/category_update","category.parent.id="+$(selected).attr("data-id")+"&category.name="+$(editor).val(),function(data){
                                		if(data!=null){
                                			if(!(data instanceof Object)) data = eval("("+data+")");
                                			if(!data.result){
                    ...
                    if (selected.length <= 0) {
                       /* $(easyTree).prepend(warningAlert);
                    	...
                    	parent.layer.confirm('确认要执行删除操作吗？', {
                    		...
                    		//删除分类
                        	$.post("json/category_delete","category.id="+$(selected).attr("data-id"),function(data){
                        		if(data!=null){
                        			if(!(data instanceof Object)) data = eval("("+data+")");
                        			if(!data.result){
                        				/*$(easyTree).prepend(warningAlert);
