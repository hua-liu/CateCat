...
$(function() {
	...
	$("#table_list").jqGrid({
		url : "json/list/list_cate_Cate_specialShow",
		...
		rowNum : 15,
		rowList : [ 10, 15, 20, 30 ],
		colNames : [ 'id', '类型', '美食名称','特殊美食展示图','一句话描述' ],
		colModel : [
				{
					name : 'id',
					index : 'id',
...
function cateNameFmatter(cellvalue, options, rowObject) {
	if(cellvalue!=null&&cellvalue.name!=null){
		return cellvalue.name;
	}else{
		return "";
	}
}
function typeNameFmatter(cellvalue, options, rowObject) {
	if(cellvalue!=null&&cellvalue.name!=null){
		return cellvalue.name;
	}else{
		return "";
...
function imgFmatter(cellvalue, options, rowObject) {
	if(cellvalue!=null&&cellvalue.id!=null){
		return "<img class='listimg' src='img_requestImg?type=max&id="+cellvalue.id+"' width='50px' onclick='showPic(this)'>";
	}else{
		return "";
	}
}
...
function deleteData(ids) {
	$.post("json/cate_deleteSpecial", "ssCate.id=" + ids, function(data) {
		data = eval("(" + data + ")");
...
function updateForm(){
	...
	$.post("json/cate_updateSpecial",$('#sourceInforForm').serialize(),function(data) {
