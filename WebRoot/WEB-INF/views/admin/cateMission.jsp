		...
		<ul class="notes">
			<li>
				<div>
					<small>六画</small>
					<h4>美食上线审核</h4>
					<p>用于美食的自动审核，此工具会自动扫描未上线美食是否符合上线规范，符合则自动进行上线处理</p>
			...
			<li>
				<div>
					<small>六画</small>
					<h4>美食下线审核</h4>
					<p>用于美食的自动审核，此工具会自动扫描已经上线的美食是否存在规范不完整，存在则自动进行下线处理</p>
...
function startMission(type,code){
	var data = "type="+type+"&code="+code+"&state=1&id="+currentID+"&name="+(type=='autoOnline'?'资源上线审核':type=='autoOffline'?'资源下线审核':type=='clearRubbish'?'垃圾文件清理':'');
	$.post("json/mission_mission",data,function(data){
		data = eval("("+data+")");
		if(!data.message){
