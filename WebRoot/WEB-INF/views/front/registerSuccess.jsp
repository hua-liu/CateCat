...
<title>${requestScope.updateUser?'更新资料':'补充资料'}</title>
	...
	<div class="ibox-content">
		<div style="text-align:center;">
			<div style="padding:30px;">
	 				...
	 				<div class="b-logo">
							<a href="home-1.html"> <span>LoveEat</span> <br> <span>greens</span>
							</a>
						</div>
	            </div>
		...
		<h2>更多设置</h2>
		<form id="form" action="user_registMessage.action" class="wizard-big" method="post">
			...
			<fieldset>
				...
				<div class="row">
					<div class="col-sm-4">
						...
						<div class="form-group">
							<label>购物支付密码</label> <input id="payPassword" name="payPassword" value="${sessionScope.user.payPassword}"
								type="text" class="form-control" placeholder="填写购物支付密码(必填)">
						</div>
					</div>
					<div class="col-sm-8">
						<div class="form-group">
			...
			<fieldset>
				<div class="row">
					...
					<div class="col-sm-6">
						<input type="hidden" name="avatar.id" id="avatar_id" value="${sessionScope.user.avatar==null?requestScope.defaultHeadPortrait!=null?requestScope.defaultHeadPortrait[0].id:'':sessionScope.user.avatar.id}">
						<div class="col-sm-4"><label style="text-align:center">头像设置</label><img class="head-img img-circle" alt="头像预览" 
						src="img_requestImg?id=${sessionScope.user.avatar==null?requestScope.defaultHeadPortrait!=null?requestScope.defaultHeadPortrait[0].id:'':sessionScope.user.avatar.id}">
						</div>
						<label>头像选择</label>
						<ul class="system_img col-sm-8">
						<c:forEach items="${requestScope.defaultHeadPortrait}" var="av" varStatus="vs">
							<li data-id="${av.id}" class="s_img ${sessionScope.user.avatar==null?vs.count==1?'active':'':sessionScope.user.avatar.id==av}"><img src="img_requestImg?id=${av.id}"></li>
						</c:forEach>
...
<%-- 
			...
			<fieldset>
				<div>
					...
					<h2>你喜欢什么类型的美食呢？</h2>
					<ul class="lover">
						<c:forEach items="${sessionScope.categorys}" var="c">
							<li value="${c.id}" class="${sessionScope.user.hobby!=null?fn:contains(sessionScope.user.hobby,c.id)?'selected':'':''}">${c.name}</li>
						</c:forEach>
					</ul>
				</div>
			</fieldset> --%>
