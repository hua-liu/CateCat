...
$(function() {
	...
	$("#table_list").jqGrid({
		url : "json/jurisdiction_listmanager",
		...
		rowNum : 15,
		rowList : [ 10, 15, 20, 30 ],
		colNames : [ 'id','用户名','邮箱','最后一次登陆时间','当前角色'],
		colModel : [
				{
					name : 'id',
					index : 'id',
...
function deleteData(ids) {
	$.post("json/jurisdiction_cancelRoleForUser", "id=" + ids, function(data) {
...
function autoComplate(el){
	...
	+"&searchString="+el.value.trim()+"&searchString="+el.value.trim();
		$.post("json/jurisdiction_getNoRoleUsers",data,function(data) {
			$("#autoComplateContext a").remove();
...
function addManagerSure(){
	...
	$.post("json/jurisdiction_updateRoleForUser",data,function(data){
			if(!(data instanceof Object))data = eval("("+data+")");
