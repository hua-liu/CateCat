$(document).ready(function () {
 ...
 $("#payPassword").bind("change",function(){
	 if($("#payPassword").val()!='0'&&this.value=='')
		 popover("支付密码不能为空",this)
	 else if(this.value.length>6)
		 popover("支付密码过长(8位)",this);
	 else
		 clearError($("#payPassword"));
 })
 ...
 function checkExist(type,el){
 	if(el.value=='')return;
$.post("json/user_checkoutExist","type="+type+"&value="+el.value,function(data) { // 此处可以
		data = eval("("+data+")")
		if(data.message){
			var text;
...
$(el).popover("destroy");
 ...
 $(function() {
		...
		$("#download").click(function() {
	        ...
	        $.ajax({    
	            url: "json/img_uploadUserHeadPortrait",  
	            type: 'POST',    
	            data: formData,
	            async: true,    
	            cache: false,    
	            ...
	            success: function (data) {
	            	...
	            	if(data.result){
	            		...
	            		$(".head-img").attr("src","img_requestImg?id="+data.id);
	            		$(".s_img").removeClass("active");
	            	}else{
